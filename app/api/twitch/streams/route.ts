...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!userIds) {
      return NextResponse.json({ error: 'user_ids parameter is required' }, { status: 400 });
    }
    ...
    const response = await fetch(`https://api.twitch.tv/helix/streams?user_id=${userIds}&first=100`, {
      headers: {
